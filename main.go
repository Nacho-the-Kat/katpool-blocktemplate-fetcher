...
func (ks *KaspaApi) GetBlockTemplate(miningAddr string, canxiumAddr string) (*appmessage.GetBlockTemplateResponseMessage, error) {
...
func main() {
	...
	go func() {
		for {
			template, err := ksApi.GetBlockTemplate(address, config.CanxiumAddr)
			if err != nil {
